...
export const addProduct = async (req, res) => {
  try {
    ...
    if (!product) {
      return res.status(400).json({ message: "thêm khoong thành công" });
    }
...
export const getAllProducts = async (req, res) => {
  ...
  try {
    const {docs , totalDoc , totalPages} = await Product.paginate({},{...options,populate: [{ path: "categoryId", select: "name" }]})
    if (docs.length === 0) {
      return res.status(404).json({ message: "không có sản phẩm nào" });
    }
    return res.status(200).json({ data:  docs, totalDoc, totalPages });
  } catch (error) {
    return res.status(400).json({ message: error.message });
  }
};
...
export const updateProduct = async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res.status(errors.length).json({ message: errors });
    }
